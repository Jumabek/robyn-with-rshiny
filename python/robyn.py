...
class Robyn(object):
    ...
    def mmm(self,
        ...
        if fixed_out:
            mod_out = self.refit(x_train, y_train, lower_limits, upper_limits, lambda_=cvmod[10])

            lambda_ = cvmod[10]
        else:
            mod_out = self.refit(x_train, y_train, lower_limits, upper_limits, lambda_=cvmod[0][i])
            lambda_ = cvmod[0][i]
